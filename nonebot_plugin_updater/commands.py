...
async def _(bot: Bot, event: Event, plugin_name: Match[str] = AlconnaMatch('plugin_name')) -> None:
    if plugin_name.available:
        ...
        if plugin_name.result == 'all':
            ...
            await update_plugin.send('正在更新插件中……')
            _save_restart_state(bot, event, f"✨ 插件 {plugin_name.result} 更新完成，重启成功！")
...
async def _(bot: Bot, event: Event, plugin_name: Match[str] = AlconnaMatch('plugin_name')) -> None:
    if plugin_name.available:
        ...
        if plugin_name.result in [plugin.project_link for plugin in store_plugins]:
            ...
            _save_restart_state(bot, event, f"✨ 插件 {plugin_name.result} 安装完成，重启成功！")
...
async def _(bot: Bot, event: Event, plugin_name: Match[str] = AlconnaMatch('plugin_name')) -> None:
    if plugin_name.available:
        ...
        if plugin_name.result in [plugin.project_link for plugin in store_plugins]:
            ...
            _save_restart_state(bot, event, f"✨ 插件 {plugin_name.result} 卸载完成，重启成功！")
            updater: Updater = Updater([], plugin_name.result)
